...
const tabber = {
	...
	init: function()
	{
		let tab_containers = document.querySelectorAll('[role="tab-container"]');
	...
	hide_all: function()
	{
		...
		for (var x = 0; x < tabber.tabs.length; x++) {
			let active_class = tabber.tabs[x].getAttribute('active-class');
			tabber.tabs[x].setAttribute('active','false');

			if(active_class)
				tabber.tabs[x].classList.remove(active_class);
		}
...
window.addEventListener('load',tabber.init);
